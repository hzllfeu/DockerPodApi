...
def safe_download(url: str, max_seconds: int, request_id: str) -> tuple[Path, WavInfo]:
    ...
    with httpx.Client(timeout=timeouts, follow_redirects=False) as client:
        ...
        # Accept the strict audio/wav types AND application/octet-stream
        # (used by GitHub raw / S3 / many CDNs for large or unknown files).
        # The actual WAV format is enforced after download by magic bytes
        # and fmt chunk parsing.
        if ct not in ("audio/wav", "audio/x-wav", "application/octet-stream",
                       "binary/octet-stream", ""):
