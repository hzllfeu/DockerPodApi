...
def _vram_ok() -> tuple[bool, float]:
    """Returns (ok, used_ratio). If VRAM_REFUSE_RATIO >= 1, always OK."""
    if VRAM_REFUSE_RATIO >= 1.0:
        return True, 0.0
    try:
        import torch
...
async def lifespan(app: FastAPI):
    ...
    # If Modal pre-loaded via _eager_load_models() before taking the memory
    # snapshot, PIPELINE is already on GPU — skip the load step.
    if PIPELINE is None or not PIPELINE.is_ready():
        ...
        log.info("pipeline already loaded (Modal snapshot path)")
    JOBS = JobManager()
